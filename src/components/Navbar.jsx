...
const Navbar = ({ currentView, onBackHome }) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // 1. Abaikan kesan "bounce" kat bahagian paling atas telefon
      if (currentScrollY <= 10) {
        setIsVisible(true);
        return;
      }

      // 2. Cuma sorok/tunjuk kalau pergerakan scroll lebih dari 5px (tak terlalu sensitif)
      if (Math.abs(currentScrollY - lastScrollY) > 5) {
        if (currentScrollY > lastScrollY) {
          setIsVisible(false); // Scroll turun = Sorok
        } else {
          setIsVisible(true);  // Scroll naik = Tunjuk semula
        }
  ...
  return (
    <nav className={`glass-nav sticky top-0 z-40 border-b border-slate-200/60 shadow-sm transition-transform duration-300 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* TUKAR: Pakai flex-row supaya sentiasa sebelah-menyebelah. Ketinggian (h-16) dilaraskan untuk telefon */}
        <div className="flex flex-row items-center justify-between h-16 sm:h-20 py-2 sm:py-0">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            {/* TUKAR: Saiz logo (h-8) dikecilkan sikit di telefon supaya muat dengan tajuk */}
